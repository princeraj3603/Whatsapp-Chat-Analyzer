...
def emoji_helper(selected_user,df):
    ...
    emojis = []
    for message in df['message']:
        emojis.extend([emojis.extend([c for c in message if emoji.is_emoji(c)])])
...
def week_activity_map(selected_user,df):
    ...
    week_activity_map = df['day_name'].value_counts()

    return week_activity_map

def month_activity_map(selected_user,df):
    if selected_user != 'Overall':
        df = df[df['user'] == selected_user]

    month_activity_map = df['month'].value_counts()

    return month_activity_map
